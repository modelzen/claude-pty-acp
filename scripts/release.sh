#!/usr/bin/env bash
# 发布 claude-pty-acp 到 npm —— 幂等、不会重复发、自动打 tag。
#
# 逻辑：
#   1. 前置门：装了 npm、已 npm login、工作区无未提交(已跟踪)改动。
#   2. 质量门：typecheck / build（无单测套件，test-client.mjs 是手动探针）。
#   3. 选版本：当前 package.json 版本若「npm 上已存在」→ 从 max(本地,线上) 自动
#      patch 递增到第一个 npm 上不存在的号；否则直接发它。发布前二次校验。
#   4. 发布：npm version 打 commit+tag → npm publish → 有 git remote 才 push
#      → 装了 gh 再建 GitHub Release（失败不影响已发布的 npm 包）。
#
# 用法：
...
if [ -n "$(git status --porcelain -uno)" ]; then
  git status -s -uno
  # dry-run 无副作用，工作区脏不该拦演练；正式发布才硬性要求干净。
  if [ "$DRY" = 1 ]; then
    c 33 "  ⚠ 有未提交的(已跟踪)改动；--dry-run 仅演练，继续（正式发布前请先 commit）"
  else
...
# push + GitHub Release：都要有 git remote 才有意义（不影响已发布的 npm 包）
if git remote | grep -q .; then
  ...
  c 90 "  未配置 git remote，跳过 push 与 GitHub Release（tag $TAG 已在本地）"
fi
